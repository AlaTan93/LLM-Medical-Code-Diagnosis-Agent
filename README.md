# LLM-Medical-Code-Diagnosis-Agent
...
`medicoder` always points its OpenAI-compatible client at the local LiteLLM proxy (`LLM_BASE_URL=http://litellm:4000/v1`). LiteLLM in turn only ever routes to in-container Ollama models (`docker/litellm/config.yaml`) — there is no cloud provider option available for security reasons.
...
This project was completed using an AMD GPU. A best-effort but untested Nvidia (CUDA) GPU setup is available.
...
| Alias | Model | Notes |
| ----- | ----- | ----- |
| `deepseek-r1-medical-cot` | `mradermacher/DeepSeek-R1-Medical-COT-GGUF:Q8_0` | Medical (thinking model, default Model A) |
| `medgemma-27b-q4_k_s` | `unsloth/medgemma-27b-text-it-GGUF:Q4_K_S` | Medical diagnosis generation (default Model B + critic) |
...
The container Ollama is **fully isolated**: it publishes no host port for security reasons.
...
directly from the host. The app can expose a temporary testing endpoint that calls
LiteLLM internally. `model` is a LiteLLM alias from `docker/litellm/config.yaml`
(e.g. `medgemma-27b-q4_k_s`, `deepseek-r1-medical-cot`, `embed`):
...
This endpoint is disabled by default, uncomment this line in main.py to reenable it:
```python
...
(default k=15, configurable), then results are deduplicated and sorted by
...
medicoder/routes/icd10.py        GET /codes, POST /search — ICD-10 lookup + direct vector search (DISABLED)
medicoder/routes/llm.py          POST /test/{model} — call a LiteLLM alias (DISABLED)
medicoder/routes/code.py         POST /code — deterministic pipeline (diagnose + search_icd10) (DISABLED)
